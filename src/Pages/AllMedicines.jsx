    ...
    <div className="all-medicines-page">
      <div className="container">
        ...
        {loading ? (
          ...
          <div className="medicines-loading" style={{ color: '#e05b5b' }}>{error}</div>
        ) : !hasResults ? (
          <div className="medicines-loading">
              ...
              {letters.map((l) => (
                <div key={l} id={`letter-${l}`} className="letter-group">
                  ...
                  <div className="letter-items">
                    {grouped[l].map((p) => (
                      <Link key={p._id} to={`/product/${p._id}`} className="medicine-item">
                        <div className="medicine-item-img">
                          <img
                            src={
                              p.image?.startsWith('/uploads')
                                ? `${import.meta.env.VITE_API_URL}${p.image}`
                                : p.image || '/assets/images/best-product1.png'
                            }
                            alt={p.name}
                          />
                        </div>
