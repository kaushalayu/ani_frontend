...
function SingleProduct() {
  ...
  if (error || !product) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 0' }}>
        ...
        <Link to="/shop" className="sp-add-cart">Back to Shop</Link>
      </div>
    )
  }
  ...
  const handleAddToCart = () => {
    addToCart({
      ...
      name: product.name,
      price: displayPrice,
      pills: currentOption ? currentOption.count : null,
      img: imgSrc,
      qty: quantity,
    })
      ...
      <div className="container">
        <div className="sp-banner">
          ...
          <div className="breadcrumb">
            ...
            <span className="breadcrumb-item"><Link to="/shop">Shop</Link></span>
        ...
        <div className="sp-layout">
          ...
          <div className="sp-info-col">
            ...
            <div className="sp-price-row">
              ...
              {displayOldPrice > 0 && (
                <span className="sp-old-price">${displayOldPrice.toFixed(2)}</span>
              )}
            </div>
              ...
              <div className="sp-pills">
                ...
                <div className="sp-pills-options">
                  {product.pillsOptions.map((opt, i) => (
                    <button
                      key={opt.count}
                      className={'sp-pill-btn' + (selectedPillsIdx === i ? ' active' : '')}
                      onClick={() => setSelectedPillsIdx(i)}
                    >
                      {opt.count} Pills
                    </button>
                  ))}
            ...
            {!isLoggedIn && (
              <div style={{
                ...
                <span>
                  <Link to="/login" style={{ fontWeight: 700, color: '#92400e' }}>Login</Link> or{' '}
                  <Link to="/join-now" style={{ fontWeight: 700, color: '#92400e' }}>Register</Link> to place an order.
            ...
            <div className="sp-detail-icons">
              ...
              <div className="sp-detail-item">
                ...
                <span>Delivers In: 4-7 Working Days <Link to="/terms-of-use">Terms &amp; Conditions</Link></span>
              </div>
            </div>
          </div>
        </div>
        ...
        {product.sideEffects && (
          <section className="sp-section">
            ...
            <div className="sp-section-body">
              <p style={{ whiteSpace: 'pre-line' }}>{product.sideEffects}</p>
            </div>
          </section>
        )}
        ...
        <section className="sp-section">
          ...
          <div className="sp-section-body">
            {product.reviews?.length === 0 && (
              <p style={{ color: '#888', marginBottom: 24 }}>No reviews yet. Be the first!</p>
            )}
              ...
              <div key={i} className="sp-review">
                ...
                <div className="sp-review-body">
                  <div className="sp-review-header">
                    <div>
                      <h5>{review.name}</h5>
                      <span className="sp-review-badge verified"><i className="fa-solid fa-circle-check" /> Verified Buyer</span>
                    </div>
            ...
            <div className="sp-review-form">
              ...
              {isLoggedIn ? (
                <form onSubmit={async (e) => {
                  ...
                  <div className="sp-review-form-group">
                    ...
                    <div className="sp-review-star-select">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <i
                          key={s}
                          className={s <= reviewStars ? 'fa-solid fa-star' : 'fa-regular fa-star'}
                  ...
                  <button type="submit" className="sp-submit-review">
                    Submit Review <i className="fa-solid fa-arrow-right" />
                  </button>
                </form>
              ) : (
                <div style={{ background: '#f3f4f6', borderRadius: 8, padding: '16px 20px', fontSize: 14, color: '#374151' }}>
                  <Link to="/login" style={{ fontWeight: 700, color: '#4f46e5' }}>Login</Link> to write a review.
                </div>
              )}
            </div>
          </div>
                  ...
                  return (
                    <div key={prod._id} className="sp-more-card">
                      <Link to={`/product/${prod.slug || prod._id}`}>
                        <div className="sp-more-img">
                          <figure><img src={pImg} alt={prod.name} /></figure>
                        </div>
                      </Link>
                      <div className="sp-more-info">
                        <span className="sp-more-type">{prod.badge || prod.category?.name}</span>
                        <h5><Link to={`/product/${prod.slug || prod._id}`}>{prod.name}</Link></h5>
                        <div className="sp-more-rating">
                          <i className="fa-solid fa-star" />
                          <span>{(prod.rating || 0).toFixed(1)}/5</span>
                        </div>
                        <div className="sp-more-footer">
                          <span className="sp-more-price">${pPrice.toFixed(2)}</span>
                        </div>
                      </div>
                    </div>
                  )
